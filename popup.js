...
function start() {
    if (document.querySelector(".gV3Svc")) {
        let count = prompt("Enter the value to auto leave (min = 2)")
        function update(msg) {
    ...
    ${msg}</div>
        ...
        update(`Waiting for ${count} users or more...`)
        ...
        function autoLeaveGoogleMeet() {
            ...
            if (document.querySelector(".gV3Svc>span").nextElementSibling.innerText < count) {
                ...
                update(`Leaving meet once active members is less than ${count}`)
                t = setTimeout(autoLeaveGoogleMeet, 5000); // here it will call itself after 5 seconds
            }
        }
        ...
        function checkBeforeStart() {
            ...
            if (document.querySelector(".gV3Svc>span").nextElementSibling.innerText > count) {
                ...
                try {
                    update(`Error: More than ${count} people are here!`)
                    autoLeaveGoogleMeet()
                } catch {
                    alert("ERROR1");
                }
